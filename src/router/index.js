...
router.beforeEach(async (to, from, next) => {
  if (to.query.code) {
    // 携带一次性登录token，清除本地登录状态，单独走独立登录接口
    localClear();
    const res = await loginApi.loginCode(to.query.code);
    ...
    // 等待路由构建完成，使用 nextTick 确保路由已注册
    await nextTick();

    // 重新匹配路由，确保路由已完全注册
    // 移除 code 参数，避免循环，保留其他查询参数
    const queryWithoutCode = { ...to.query };
    ...
    // 先尝试通过路径查找路由
    const matchedRoute = router.resolve(targetLocation);
    if (matchedRoute && matchedRoute.name !== '404') {
      const routeName = matchedRoute.name;
      let toRouterInfo = routerMap.get(routeName);
      if (toRouterInfo && _.isFunction(toRouterInfo.component) && toRouterInfo.meta.renameComponentFlag === false) {
        ...
        toRouterInfo.component().then((val) => {
          // 修改组件的name
          val.default.name = toRouterInfo.meta.componentName;
          // 记录已经修改过 组件的name
          toRouterInfo.meta.renameComponentFlag = true;
        });
      }
      // 设置tagNav
      useUserStore().setTagNav(matchedRoute, from);

      // 设置keepAlive
      if (toRouterInfo && toRouterInfo.meta.keepAlive) {
        nextTick(() => {
          useUserStore().pushKeepAliveIncludes(toRouterInfo.meta.componentName);
        });
      }
    }

    // 使用 next() 重新匹配路由，确保路由已完全注册
    // 移除 code 参数后重新导航
    next(targetLocation);
    return;
  }
  ...
  // 公共页面，任何时候都可以跳转
  if (to.path === PAGE_PATH_404 || to.path === '/collect' || to.path === '/share') {
    next();
    return;
  }
  ...
  if (!token) {
    useUserStore().logout();
    console.log("目标地址为" + JSON.stringify(to))
    if (to.path === PAGE_PATH_LOGIN) {
      next();
    } else {
      console.log("目标地址为" + to.fullPath)
      // 构建完整的 redirect 路径，包括 path、query 和 hash
  ...
  // 登录页，如果已登录且有 redirect 参数，则跳转到 redirect，否则跳转到首页
  if (to.path === PAGE_PATH_LOGIN) {
