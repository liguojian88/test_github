<template>
  <div class="body">
    ...
    <a-modal class="model" v-model:open="open" title="须知" :footer="null" :maskClosable="false">
      <ul class="ulContent">
        <li>请遵守公司政策 <a @click.stop="jumpPolicy(1)"> &lt; Acceptable Use of Tech Resources Policy(w11) &gt;</a> 和 <a
...
const submit = () => {
  addForm.value.validate((vaild, m) => {
    if (vaild) {
      ...
      let params = {
        applyId: formState.value.applyId,
        ...
        managerEmail: formState.value.managerEmail

      }
      shareApi.update(params).then((res) => {
        btnLoading.value = false
        reset()
        message.success('成功')
        router.push({
...
const getDetailData = () => {
  if (route.query.applyId) {
    shareApi.queryDetailById(route.query.applyId).then((res) => {
      ...
      // 回显文件类别
      if (detailData.file && detailData.file.fileCategory !== undefined) {
        formState.value.fileType = String(detailData.file.fileCategory)
        formState.value.file.fileCategory = detailData.file.fileCategory
      } else if (route.query.fileCategory) {
        formState.value.fileType = route.query.fileCategory
        formState.value.file.fileCategory = route.query.fileCategory
      }

      // 回显文件信息
      if (detailData.file) {
        formState.value.file = {
          documentName: detailData.file.documentName || null,
          documentKey: detailData.file.documentKey || null,
          documentUrl: detailData.file.documentUrl || null,
          fileCategory: detailData.file.fileCategory || null,
          maxDownloads: detailData.file.maxDownloads || null,
          fileInvalidTim: detailData.file.fileInvalidTime || null,
          fileSize: detailData.file.fileSize || null
        }
        formState.value.filename = detailData.file.documentName || null
      }
    }).catch((err) => {
